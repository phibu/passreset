...
<#
.SYNOPSIS
    Installs PassCore V2 on IIS (Windows Server 2022 / IIS 10).
...
.PARAMETER SiteName
    Name of the IIS site to create or update. Default: PassCore

.PARAMETER AppPoolName
    Name of the IIS application pool. Default: PassCorePool

.PARAMETER PhysicalPath
    Folder where the app will be deployed. Default: C:\inetpub\PassCore

.PARAMETER PublishFolder
    Path to the dotnet publish output folder (the folder containing PassCore.Web.exe).
...
param(
    [string] $SiteName        = 'PassCore',
    [string] $AppPoolName     = 'PassCorePool',
    [string] $PhysicalPath    = 'C:\inetpub\PassCore',
...
if (-not (Test-Path $PublishFolder)) {
    Abort "Publish folder not found: $PublishFolder`nRun: dotnet publish src\PassCore.Web -c Release -o deploy\publish"
}

$webExe = Join-Path $PublishFolder 'PassCore.Web.exe'
if (-not (Test-Path $webExe)) {
    Abort "PassCore.Web.exe not found in $PublishFolder. Ensure you ran dotnet publish first."
}
    ...
    @'
{
  ...
  "PasswordExpiryNotificationSettings": {
    "Enabled": false,
    "PassCoreUrl": "https://passcore.yourdomain.com"
  },
  "ClientSettings": {
    "UseEmail": true,
...
Write-Host '  PassCore V2 installed successfully.' -ForegroundColor Green
Write-Host ''
